...
class Camera:
    ...
    def __init__(self, serial, width, height, framerate, color, liveview, topic_name=None, node_name=None):
        ...
        gscam = 'shmsrc socket-path=/tmp/ros_mem ! video/x-raw, format=RGB, width=%d,height=%d,framerate=%d/1' % (
            width, height, framerate,)
...
def main():
    ...
    cam = Camera(str(serial_number), 640, 480, 30, True, False)
