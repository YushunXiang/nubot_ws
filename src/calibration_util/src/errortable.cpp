...
ErrorTable::getErrorTable()
{
   ...
   for(int j = starty_;j <=  endy_; j++)
   {
       for(int i = startx_; i <= endx_;i++)
       {
        ...
        //       }
        //   }
            Circle & goalcircle1 = field_info_.goalcircle_[0];
            double dis_goalcircle1 = goalcircle1.center_.distance(pt);
            dis_goalcircle1 = std::abs(dis_goalcircle1 - goalcircle1.radius_);
            if(dis_goalcircle1 < Mindis && pt.x_<255)
                Mindis = dis_goalcircle1;

            Circle & goalcircle2 = field_info_.goalcircle_[1];
            double dis_goalcircle2 = goalcircle2.center_.distance(pt);
            dis_goalcircle2 = std::abs(dis_goalcircle2 - goalcircle2.radius_);
            if(dis_goalcircle2 < Mindis && pt.x_>-255)
                Mindis = dis_goalcircle2;
