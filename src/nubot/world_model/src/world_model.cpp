...
nubot::World_Model::updateFrontVision(const nubot_common::FrontBallInfo & _front_info)
{
    ...
    frontvision_ball.setValid(false);
...
//        world_model_info_.robotinfo[i].targetNum4=robot_info.getTargetNum(4);
    ...
    // ROS_INFO("#######################--ball loc from World Model pulisher: %f , %f",world_model_info_.ballinfo[AgentID_-1].pos.x,world_model_info_.ballinfo[AgentID_-1].pos.y);
}
/**
...
nubot::World_Model::sendToCoach()
{
    ...
    robot2coach_.robot_global_loc.x_=robot_info.getLocation().x_;
    robot2coach_.robot_global_loc.y_=robot_info.getLocation().y_;

    robot2coach_.ball_global_loc.x_=ball_info_.own_ball_.getGlobalLocation().x_;
    robot2coach_.ball_global_loc.y_=ball_info_.own_ball_.getGlobalLocation().y_;
    DPoint ball_real(ball_info_.own_ball_.getRealLocation());
    robot2coach_.ball_real_loc.x_=ball_real.x_;
    robot2coach_.ball_real_loc.y_=ball_real.y_;
    robot2coach_.ball_global_vec.x_ = ball_info_.own_ball_.getVelocity().x_;
    robot2coach_.ball_global_vec.y_ = ball_info_.own_ball_.getVelocity().y_;
    ...
    for(int i=0; i < MAX_OBSTALCES_NUMS_CONST; i++)
    {
        if (i<tracker.size())
        {
            robot2coach_.otherRobot[i].x_=tracker[i].x_;
            robot2coach_.otherRobot[i].y_=tracker[i].y_;
        }
        else
        {
            robot2coach_.otherRobot[i].x_=-10000;
            robot2coach_.otherRobot[i].y_=-10000;
        }
    }
...
#else
    DB_put(MESSAGETOCOACHINFO, &robot2coach_);
#endif
}
