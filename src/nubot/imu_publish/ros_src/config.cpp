...
unsigned char  InitConfig(const char* fileName){

    int status = 0;
    const cJSON *name = NULL;
    const char * pJSONData = cJSON_ReadFileToMem(fileName); //"/home/atom/catkin_ws/devel/lib/imu_publish/cfg.json")
...
unsigned char  IMU_InitConfig(const char* fileName){
    ...
    ConfigSingleDataPacket cfgPacket[DATAPACKET_CNT_ROS];
#else
    ConfigSingleDataPacket IMU_cfgPacket[DATAPACKET_CNT];
#endif
...
#endif
        pid = SESSION_NAME_LINEAR_ACC | 0x8000;
        IMU_cfgPacket[cfgIndex].reserve0 = 0xff;
        IMU_cfgPacket[cfgIndex].reserve1 = 0xff;
        IMU_cfgPacket[cfgIndex].packetID = pid;
        cfgIndex++;
        dataPktLen+= LINEARACC_LEN;
    }
    ...
    if (cJSON_IsTrue(name))
    {
        ...
        printf("Acceleration rawdata enable \n");
#endif
        pid = IMU_SESSION_NAME_RAW_ACC | 0x8000;
        IMU_cfgPacket[cfgIndex].reserve0 = 0xff;
        IMU_cfgPacket[cfgIndex].reserve1 = 0xff;
        IMU_cfgPacket[cfgIndex].packetID = pid;
        cfgIndex++;
    ...
    {
        printf("Gyroscope rawdata enable \n");
        pid = IMU_SESSION_NAME_RAW_GYRO | 0x8000;
        IMU_cfgPacket[cfgIndex].reserve0 = 0xff;
        IMU_cfgPacket[cfgIndex].reserve1 = 0xff;
        IMU_cfgPacket[cfgIndex].packetID = pid;
        cfgIndex++;
...
#endif
    SetDataPacketConfigReq(nFD, (u8 *) &IMU_cfgPacket, cfgIndex * 4);
    SwitchModeReq(nFD, MEASURE_MODE);
    /*TODO:free pJSONData*/
    if(pJSONData != NULL){
        free((void *)pJSONData);
