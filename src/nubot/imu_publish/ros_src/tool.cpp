...
int IMU_Align(unsigned char nFD)
{
    unsigned char tmp = 0;
    //unsigned char tmp1 = 0;
    unsigned int uLen = 0;
...
bool ValidFrame(unsigned char * tmpBuf,int frameLen)
{
    return  (tmpBuf[0] == 0x41) &
            (tmpBuf[1] == 0x78) &
            (tmpBuf[2] == 0xFF) &
            (tmpBuf[frameLen-1] == 0x05);
}
bool IMU_ValidFrame(unsigned char * tmpBuf,int frameLen)
{
    return  (tmpBuf[0] == 0x41) &
            (tmpBuf[1] == 0x78) &
            (tmpBuf[2] == 0xFF) &
...
int  IMU_FillFrameHead(unsigned char * tmpBuf)
{
    tmpBuf[0] = 0x41;
    tmpBuf[1] = 0x78;
    tmpBuf[2] = 0xFF;
...
void ParserDataPacket(Data *DataHandle,u8 *pBuffer, u16 dataLen, FILE *fpLog)
{
    ...
    while (index < dataLen)
    {
        ...
        pl = *(pData + index + 1);
     //   printf("PID=%x",PID);
        if (PID == (SESSION_NAME_TEMPERATURE))//上一次上位机fa 送的控制信息
        {
            //Ignore pid and pl
            index += 2;
//printf("heeeeeee\n");
        ...
        else if (PID == (SESSION_NAME_RAW_ACC))//车体实际速度信息
        {
            //Ignore pid and pl
            index += 2;
//printf("hot\n");
        ...
        else if (PID == SESSION_NAME_RAW_GYRO)//轮速信息
        {
            //Ignore pid and pl
            index += 2;
//printf("hog\n");

            memcpy(&DataHandle->gyroRawData.gyroX, pData + index, PL_CAL_DATA);
        ...
        else if (PID == SESSION_NAME_RAW_MAG) //
        {
            ...
            index += 2;
//printf("heg\n");
            memcpy(&DataHandle->status2.cPower, pData + index, PL_KAL_DATA);
            DataHandle->status2.dataID = PID;
            DataHandle->status2.dataLen = pl;
            index += PL_KAL_DATA;
        }
...
void IMU_ParserDataPacket(Data *DataHandle,u8 *pBuffer, u16 dataLen, FILE *fpLog)
{
    u16 PID = 0;
    u8 *pData = pBuffer;
    u8 index = 0;
...
int k=0;
    ...
    while (index < dataLen)
    {
        ...
        pl = *(pData + index + 2);
        printf("PID=%x",PID);
        if (PID == (IMU_SESSION_NAME_TEMPERATURE))//上一次上位机fa 送的控制信息
        {
            //Ignore pid and pl
            index += 3;
printf("heeeeeee\n");
            memcpy(&DataHandle->temperature.data, pData + index, IMU_PL_TEMPERTURE);
            DataHandle->temperature.dataID = PID;
            DataHandle->temperature.dataLen = pl;
            //printf(" *** temperature:\t%11.4f *** \n", DataHandle->temperature.data);

            index += IMU_PL_TEMPERTURE;

        }
        else if (PID == (IMU_SESSION_NAME_RAW_ACC))//车体实际速度信息
        {
            //Ignore pid and pl
            index += 3;
printf("hot\n");

            memcpy(&DataHandle->accRawData.accX, pData + index, IMU_PL_RAW_DATA);
            DataHandle->accRawData.dataID = PID;
            DataHandle->accRawData.dataLen = pl;

            index += IMU_PL_RAW_DATA;

        }
         else if (PID == (IMU_DISTANCE))//里程计
        {
            //Ignore pid and pl
            index += 3;
printf("hot\n");

            memcpy(&DataHandle->distance.distanceX, pData + index, IMU_PL_CAL_DATA);
            DataHandle->distance.dataID = PID;
            DataHandle->distance.dataLen = pl;

            index += IMU_PL_CAL_DATA;

        }
        else if (PID == IMU_SESSION_NAME_RAW_GYRO)//轮速信息
        {
            //Ignore pid and pl
            index += 3;
printf("hog\n");

            memcpy(&DataHandle->gyroRawData.gyroX, pData + index, IMU_PL_RAW_DATA);
            DataHandle->gyroRawData.dataID = PID;
            DataHandle->gyroRawData.dataLen = pl;
            index += IMU_PL_RAW_DATA;
        }
        else if (PID == IMU_SESSION_NAME_RAW_MAG) //击球杆高度信息
        {
            //Ignore pid and pl
            index += 3;
printf("heg\n");
            memcpy(&DataHandle->magRawData.magX, pData + index, IMU_PL_RAW_DATA);
        ...
        else if (PID == SESSION_NAME_CAL_ACC)
        {
            ...
            memcpy(&DataHandle->accCal.accX, pData + index, IMU_PL_CAL_DATA);
            DataHandle->accCal.dataID = PID;
            DataHandle->accCal.dataLen = pl;
            index += IMU_PL_CAL_DATA;
            if(fpLog!=NULL)
        ...
        else if (PID == SESSION_NAME_CAL_GYRO)
        {
            ...
            memcpy(&DataHandle->gyroCal.gyroX, pData + index, IMU_PL_CAL_DATA);
        ...
        else if (PID == SESSION_NAME_CAL_MAG)
        {
            ...
            memcpy(&DataHandle->magCal.magX, pData + index, IMU_PL_CAL_DATA);
            DataHandle->magCal.dataID = PID;
            DataHandle->magCal.dataLen = pl;
            index += IMU_PL_CAL_DATA;
        ...
        else if (PID == SESSION_NAME_KAL_ACC)
        {
            ...
            memcpy(&DataHandle->accKal.accX, pData + index, IMU_PL_KAL_DATA);
            DataHandle->accKal.dataID = PID;
            DataHandle->accKal.dataLen = pl;
            index += IMU_PL_KAL_DATA;
            if(fpLog!=NULL)
                fprintf(fpLog," *** accKal:     \t%11.4f, %11.4f, %11.4f *** \n", DataHandle->accKal.accX, DataHandle->accKal.accY, DataHandle->accKal.accZ);
        }
        else if (PID == SESSION_NAME_KAL_GYRO)
        {
            ...
            memcpy(&DataHandle->gyroKal.gyroX, pData + index, IMU_PL_KAL_DATA);
            DataHandle->gyroKal.dataID = PID;
            DataHandle->gyroKal.dataLen = pl;
            index += IMU_PL_KAL_DATA;
        }
        else if (PID == SESSION_NAME_KAL_MAG)
        {
            ...
            memcpy(&DataHandle->magKal.magX, pData + index, IMU_PL_KAL_DATA);
            DataHandle->magKal.dataID = PID;
            DataHandle->magKal.dataLen = pl;
            index += IMU_PL_KAL_DATA;
        }
