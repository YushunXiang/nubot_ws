...
ScanPoints::ScanPoints(OmniImage & _omni_img){
    ...
    // ROS_INFO_STREAM(endpt.x_<<" "<<endpt.y_<<" "<<startpt.x_<<" "<<startpt.y_);
...
{ 
	...
	if (endPt.x_!=startPt.x_ && endPt.y_!=startPt.y_) 
	{	
		while(endPt.distance(tmpScanpt)>distance)//! y increase faster than x
		{
            if(std::abs(scanLine.k_)>1)
			{
				tmpScanpt.y_=tmpScanpt.y_+direction.y_;
    ...
    {
        while(std::abs(startPt.x_-tmpScanpt.x_)<distance)
        {
            tmpScanpt.x_= tmpScanpt.x_+direction.x_;
        }
        while (std::abs(endPt.x_-tmpScanpt.x_)>distance)
		{
		    tmpScanpt.x_= tmpScanpt.x_+direction.x_;
            DPoint2i ljt=DPoint2i(tmpScanpt.x_,startPt.y_);
            if(ljt.distance(Small_ROI.center_)>Small_ROI.radius_)
            {
               polar_pt.push_back(tmpScanpt);
            }
		}
	...
	{
        while(std::abs(startPt.y_-tmpScanpt.y_)<distance)
        {
            tmpScanpt.y_= tmpScanpt.y_+direction.y_;
        }
        while (std::abs(endPt.y_-tmpScanpt.y_)>distance)
	 	{			
			tmpScanpt.y_= tmpScanpt.y_+direction.y_;
            DPoint2i wth=DPoint2i(startPt.x_,tmpScanpt.y_);
            if(wth.distance(Small_ROI.center_)>Small_ROI.radius_)
            {
               polar_pt.push_back(tmpScanpt);
            }
		}
...
#pragma omp parallel for
	...
	for(size_t i = 0; i < nums_polars; i++)
	{
        std::vector<uchar> & ColorY=polar_pts_y_[i];
        //ColorH.resize(ColorY.size()); 
        omni_img_->getColorValues(polar_pts_[i],ColorY,0,SELECTION_YUV);
    }
	size_t nums_horizs=horiz_pts_.size();
#ifdef using_openmp
