...
public:
    ...
    void update_my_dribble_info(const nubot_common::OdoInfo & _my_msg)
    {
        ...
        ROS_INFO("is_holdball :  %d",m_dribble_.is_dribble_);                                                          //**
    }
#endif
    void update_world_model_info(const nubot_common::WorldModelInfo & _world_msg)
    {        
        // for(std::size_t i = 0 ; i < OUR_TEAM ; i++)
        // ROS_INFO("??????????????????????????????????????-BALL loc from Nubot Control: %f , %f",_world_msg.ballinfo[i].pos.x ,_world_msg.ballinfo[i].pos.y);
        ...
        ROS_INFO("world_model_info_.Obstacles_ : %d",_world_msg.obstacleinfo.pos.size());
        ...
        ROS_INFO("world_model_info_.Opponents_ : %d",_world_msg.oppinfo.pos.size());
        /** 更新足球物信息*/
        for(std::size_t i = 0 ; i < OUR_TEAM ; i++)
        {
       ...
       /*else if(match_mode_ == STARTROBOT)// 机器人正式比赛了，进入start之后的机器人状态
        {
            ...
            /////
	    ROS_INFO("match_mode_ == STARTROBOT,%d",match_mode_);
	    /////
...
//////////////////////////////////////////////////////////////////////////////////////
	else if (match_mode_ == STARTROBOT)
	{
		m_normalgame_->update();
		//bool KICKOFF = 0;
		ROS_INFO("/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////");
		ROS_INFO("KICKOFF=%d",KICKOFF);
	ROS_INFO("/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////");	
		if(!KICKOFF)
		{
			...
			if(world_model_info_.AgentID_ == 1)
			{
				m_normalgame_ -> process(OURKICKOFF);
			}
			if (m_normalgame_ -> kick_enable_)
			{
				setShoot(6.0);
...
//        m_plan_.m_behaviour_.last_app_w_  = m_plan_.m_behaviour_.app_w_;
        ...
        ROS_INFO("w&last_w: %f, %f", m_plan_.m_behaviour_.app_w_,m_plan_.m_behaviour_.last_app_w_);
        ROS_INFO("V: %f, %f, %f", vx,vy,w);
	double sqare2 = vx*vx + vy*vy;
        ...
        ROS_INFO("commandV: %d, %d, %f", command.Vx, command.Vy, command.w);
...
#else
        shoot.request.strength = 100.0;//6.0
#endif
