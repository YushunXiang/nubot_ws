...
//-----------------这是为了实现Eight_Attack而创建的全局的列表 ,但是这个其实没啥意义，是我们一次调车
#define REACHED 40
#define BALLNEAR 150
DPoint eight_p[8];

int eightpointer = 0; //记录下一个要到达的点
...
//////////////////////////////////////////////
    ...
    if (selected_action_ == AtShootSituation) //踢球
    {
        ...
        m_plan_->m_behaviour_.clearBehaviorState();  //2020.12.9
        kick_enable_ = true;
    }
    else
    {
...
void NormalGame::ourKickoff()
{
	...
	if(selected_action_ == Catch_Positioned)
	{
		m_plan_ -> catchBall();
		selected_action_ = AtShootSituation;
		target = DPoint(world_model_ -> RobotInfo_[world_model_ ->AgentID_ - 1 + 1].getLocation());
...
{
    ROS_INFO("1v1\n");
    //m_plan_->m_behaviour_.setAppw(100);//2020.11.17
    //m_plan_->catchBall();
    //selected_action_ = Eight_Attack;
    ROS_INFO("selected_action: %d", selected_action_);

    DPoint target, br;
    target = DPoint(160 , 41);
    br = target - robot_pos_;
    //m_plan_->positionAvoidObs2(br.angle().radian_);
    ...
    {
        // m_plan_->m_behaviour_.setAppw(100);
        ROS_INFO("has catched ");
    ...
    if (selected_action_ == Positioned) //移动到目标位置 move to position 射门位置
    {
        ...
        br = target - robot_pos_;
       // m_plan_->positionAvoidObs2(br.angle().radian_);
        m_plan_->move2Positionwithobs_withball(target);
        if (robot_ori_.radian_ +0.1< (oppGoalPosl - robot_pos_).angle().radian_ && robot_ori_.radian_ -0.1> (oppGoalPosr - robot_pos_).angle().radian_)
        {
            m_plan_->move2Positionwithobs_withball(target);  //之前为noball
            if(robot_pos_.distance(target)<50)//之前没有，，现在加上对距离的判断
            selected_action_ = AtShootSituation;
	    else ROS_INFO("has catched ");
        }
        else
        {
            br = DPoint(200,41)-robot_pos_;
	        m_plan_->positionAvoidObs2(br.angle().radian_);
        }
     }
...
void NormalGame::test()
{
    ROS_INFO("TEST NODE");
    if(selected_action_ == AtShootSituation)return;
    selected_action_ = Positioned;
    FieldInformation m_field_;
    if(m_field_.isInOuterRect(robot_pos_,10))
    {
        selected_action_=No_Action;
    }
    if (selected_action_ == Positioned)
    {
        m_plan_->move2Positionwithobs_noball(ball_pos_);
    }
    if ((robot_pos_.distance(ball_pos_) < BALLNEAR) && (selected_action_ == Positioned))
    {
        m_plan_->catchBall();
    }
    if ((m_dribble_->is_dribble_) && (selected_action_ == Positioned))
    {
        DPoint target(200, -41);
        m_plan_->move2Positionwithobs_withball(target);
        if (robot_pos_.distance(target) < REACHED)selected_action_ = Positioned_Static;
    }
    if (selected_action_ == Positioned_Static)
    {
        DPoint target(300, 0);
        m_plan_->positionAvoidObs(target);
        if (m_dribble_->is_dribble_ == 0)selected_action_ = Positioned;
    }
    if ((robot_ori_.radian_ + 0.1 < (oppGoalPosl - robot_pos_).angle().radian_ && robot_ori_.radian_ - 0.1 > (oppGoalPosr - robot_pos_).angle().radian_)&&(selected_action_== Positioned_Static))
    {
        selected_action_ = AtShootSituation;
    }
}
...
//射门
{
    ...
    ROS_INFO("1v1\n");
    ROS_INFO("selected_action: %d", selected_action_);
    ROS_INFO("robot_pos_X,robot_pos_Y: %f,%f", robot_pos_.x_,robot_pos_.y_);
    ROS_INFO("ball_pos_X,ball_pos_Y: %f,%f", ball_pos_.x_,ball_pos_.y_);

    DPoint target, br;
    target = DPoint(0,0);  //3 代车等球位置   
    double dist_ball = robot_pos_.distance(ball_pos_);  //2020.12.10
    ROS_INFO("distace_ball: %f", dist_ball);
    
    br = target - robot_pos_;
    double dist_target = robot_pos_.distance(target);  //2020.12.10
    ...
    else if(dist_target < REACHED)
    {
        selected_action_ = NULLaction;
        hasReached = true;
        m_plan_->move2Positionwithobs_withball(target);
    }
    else
    ...
    if( (hasReached == true) && (dist_ball < BALLNEAR) && (m_dribble_->is_dribble_==false) )  
    {
        selected_action_ = Catch_Positioned;
    }

    if (selected_action_ == Catch_Positioned)    //球来了，           //走到球的位置并完成抓球
    {
        m_plan_->catchBall();
    }
    if (selected_action_ == Positioned) //移动到目标位置 move to position 射门位置
    {
        ...
        target = DPoint(200,41);
        br = target - robot_pos_;
        ROS_INFO("Robot_Angle: %f",robot_ori_.radian_ );
        ROS_INFO("Angle: %f", br.angle().radian_);
        m_plan_->move2Positionwithobs_withball(target);
        ...
        if (robot_ori_.radian_ + 0.1 < (oppGoalPosl - robot_pos_).angle().radian_ && robot_ori_.radian_ - 0.1 > (oppGoalPosr - robot_pos_).angle().radian_)
        {
            ...
            if(robot_pos_.distance(target)<50)//之前没有，，现在加上对距离的判断
            selected_action_ = AtShootSituation;
	        else ROS_INFO("I HAVE GOT THE BALL and GO TO Shoot_Position");
        }
        else
        {
            br = DPoint(200,41)-robot_pos_;
...
void NormalGame::update()
{
    ...
    //    }
    ROS_INFO("isdribble:  %d", m_dribble_->is_dribble_); //**
    if (m_dribble_->is_dribble_)
    {
