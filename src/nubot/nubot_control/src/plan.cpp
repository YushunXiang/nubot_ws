...
void Plan::catchMotionlessBall(DPoint target, double maxvel, double maxw)  //抓住静止的球
{
    if(m_dribble_->is_dribble_ != true)
    {
        ...
        positionAvoidObs2(br.angle().radian_, maxw, 5*DEG2RAD);  //朝向目标位置
        if(robot_pos_.distance(target) < LIMITDRIBLLEDIS)
        {
            if(fabs(br.angle().radian_-robot_ori_.radian_)<4*DEG2RAD)  //5
            {
                move2Positionwithobs_noball(target,10.0,maxvel);   //移动到目标位置
                ROS_INFO("continue move!!!");
            }
        }
        else
        {
...
void Plan::positionAvoidObs2(double targetangle, double maxw, double angle_thres)    //朝向目标位置
{
    ...
    {
        ROS_INFO("need to rotate,delta_angle = %lf>%lf",fabs(targetangle-robot_ori_.radian_),angle_thres);
        m_behaviour_.rotate2AbsOrienation(KA_MOVE,KB_MOVE,targetangle,maxw,robot_ori_.radian_, angle_thres);
    }
    else
    {
        ROS_INFO("unneed to rotate");
        m_behaviour_.setAppw(0);
    }
}

void Plan::move2Positionwithobs_withball(DPoint target, double distance_thres, float maxvel, float maxacc, bool avoid_ball)  //移动到目标位置
{

    
    DPoint tem=target-robot_pos_;//
    positionAvoidObs(target);
    if(fabs(target.distance(robot_pos_))>=distance_thres)
    {
        
        double fabsAngleError=fabs(tem.angle().radian_-robot_ori_.radian_);
        if(fabsAngleError<5.0/180*3.14)
        {
        ...
        {//______________-jjx的修改
            m_subtargets_.robot_pos_=robot_pos_;
            m_subtargets_.ball_pos_=ball_pos_;
            m_subtargets_.subtarget(target, robot_pos_, avoid_ball);
            m_behaviour_.move2Position(KA_MOVE,KB_MOVE,m_subtargets_.subtargets_pos_,maxvel,robot_pos_,robot_ori_);
            m_behaviour_.setAppvx(m_behaviour_.app_vx_/fabsAngleError/11.5);
            m_behaviour_.setAppvy(m_behaviour_.app_vy_/fabsAngleError/11.5);//用偏差的角度来控制速度的大小。
        }
