...
/*2020.11.24---------------------因为这两个函数好像没什么用　所以注释了——－－－－－－－－－－－－－－－－－－－－－－－－－
...
void Behaviour::move2target(float pval, float dval,DPoint targetVector, DPoint realtarvel/*目标速度*/, float maxvel,
                 const DPoint  & _robot_pos,const Angle  & _robot_ori)//预测target的下一步的位置　并调用move2position
{
    DPoint after_target;
    double time=targetVector.norm()*2/maxvel;
    after_target.x_=realtarvel.x_*time, after_target.y_=realtarvel.y_*time;
    after_target+=targetVector;
...
/////////////////
// rotate to the target orientation by using PD control、、传入一个角度
//!!!!!!!!!!!!!!!!!!!旋转的比例因子乘以了1.5 11.28 2020
void Behaviour::rotate2AbsOrienation(float pval, float dval, float orientation,float maxw,const Angle & _robot_ori, double angle_thres)//自身坐标系
{
    ...
    std::cout<<"behaviour　2angle: "<< orientation <<' '<<_robot_ori.radian_<<std::endl;
...
//std::cout<<"app_w: "<<app_w_<<std::endl;
}
/*
