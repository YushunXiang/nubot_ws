...
#define linearVel_X 3
#define linearVel_Y 4
#define angularVel 0 // direction <0:clockwise >0:counter-clockwise
...
void TeleopNubot::joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    if(joy->buttons[Button_Y])  //左手柄  前后左右
    {
        strength += 0.2;	//右手柄  转动
    ...
    // double velnowx = 0;
    // double velnowy = 0;
    cmd.Vx = joy->axes[linearVel_X]*-100;  //-100 ~ 100
    cmd.Vy = joy->axes[linearVel_Y]*100;
    cmd.w  = joy->axes[angularVel]*-100;
